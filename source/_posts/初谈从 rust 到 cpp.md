...
rust 充分吸取了这个教训，彻底消除了空引用这个设计，换用 Option<T> 枚举。跟上文的 Result<T, E> 类似， Option<T> 枚举也需要穷尽匹配，强制要处理值为 None 时的情况。经过 rust 的锻炼，我相信程序员在转写 cpp 时，也能做到更有意识地考虑变量为空值时的情况（虽然这也本来就是应该的）。
