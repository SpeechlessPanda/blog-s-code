---
title: 友链
date: 2026-03-01 14:35:00
type: link
aside: false
---

欢迎交流与互访，想要交换友链的请在评论区或通过邮箱联系我。

## 个人博客

{% flink %}
- class_name:
  class_desc:
  link_list:
    - name: 寒夜雨
      link: https://www.coderlock.site
      avatar: https://www.coderlock.site/doomee.png
      descr: 汀苇乱摇寒夜雨，沙鸥闲弄夕阳天。
    - name: Wh1isper's Blog
      link: https://blog.wh1isper.top/
      avatar: https://blog.wh1isper.top/img/avatar-cyberpunk.png
      descr: ya-mono harness 的开发者，AI agent 领域大佬。
    - name: Vaala Cat
      link: https://vaala.cat/
      avatar: https://vaala.cat/favicon.ico
      descr: frp-panel 与 vaala ai 的开发者，字节飞书优秀员工。
    - name: jyi
      link: https://jyi2ya.github.io/
      avatar: https://avatars.githubusercontent.com/u/86813521?v=4
      descr: rvs 机器人的主人，博客里有可爱能互动的 rvs 3D 模型。
    - name: Saurlax's Blog
      link: https://saurlax.com/
      avatar: https://avatars.githubusercontent.com/u/49343742?v=4
      descr: 一位更新博客非常多的大佬，博客里 pinned 了很多好用的工具和框架。
    - name: Vaaandark's Blog
      link: https://vaaandark.top/
      avatar: https://vaaandark.top/image/vaaandark.png
      descr: 技术博客更新极多，涉猎广泛，干货满满。
    - name: Gaster's Blog
      link: https://wdgaster703.github.io
      avatar: https://wdgaster703.github.io/img/about/WDGaster.jpg
      descr: 喜欢收集知识涉猎广泛的电气学长。
    - name: Kowyo's Blog
      link: https://kowyo.com/
      avatar: https://avatars.githubusercontent.com/u/110339237?v=4
      descr: HOA 主要维护者和创始人之一， mini-agent 开发者。
    - name: YinMo19's Blog
      link: https://blog.yinmo19.top/
      avatar: https://blog.yinmo19.top/avatar_hu_371a52f5d2172aa2.png
      descr: OSA 前会长，逆向领域大神， zeroreader 开发者。
    - name: 科技爱好者周刊
      link: https://www.ruanyifeng.com/blog/index.html
      avatar: https://www.ruanyifeng.com/blog/images/person2_s.jpg
      descr: 记录每周值得分享的科技内容，周五发布。
{% endflink %}

## 有用资源

{% flink %}
- no_avatar: true
  class_name:
  class_desc:
  link_list:
    - name: HITSZ 课程攻略共享计划
      link: https://hoa.moe
      descr: HITSZ 学生必须知道的攻略网站。
    - name: 学习 C++
      link: https://learncpp.cn/
      descr: LearnCpp.com 中文版，从入门到精通的现代 C++ 免费教程。
    - name: Battle-Tested Patterns
      link: https://totoro-jam.github.io/battle-tested-patterns/zh/patterns/
      descr: 46 个生产验证的编程模式，含交互可视化与多语言实现。
    - name: Typst 中文教程
      link: https://typst-doc-cn.github.io/tutorial/introduction.html
      descr: 面向所有 Typst 用户的中文教程，与官方文档互补。
    - name: AIInfraGuide
      link: https://caomaolufei.github.io/AIInfraGuide/
      descr: 从 CUDA 编程到分布式训练与推理优化的 AI Infra 全栈知识库。
    - name: 你缺失的那门计算机课
      link: https://www.criwits.top/missing/
      descr: 适合电脑小白入门的电脑使用课程，面向信息时代的指南。
    - name: CS 自学指南
      link: https://csdiy.wiki/
      descr: 北大学长整理的计算机自学指南，汇总国内外高质量 CS 课程。
{% endflink %}

## 本站信息

名称：SpeechlessPanda - A blog
地址：https://speechlesspanda.github.io
头像：https://speechlesspanda.github.io/img/avatar.png
介绍：分享一只无语熊猫的观点、经验和生活思考，偶尔也会讲一些技术相关内容。
